    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]" onClick={onClose}>
      ...
      >
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${r.isCo ? 'bg-success-dim text-success' : 'bg-secondary text-text-muted'}`}>
                  {r.isCo ? 'C/O' : t('proj.detail.nonCo')}
                </span>
        ...
        <div className="px-4 py-2 border-t border-border flex items-center gap-4 text-[10px] text-text-subtle">
          <span><kbd className="bg-secondary px-1 py-0.5 rounded font-mono">↑↓</kbd> {t('search.move')}</span>
          <span><kbd className="bg-secondary px-1 py-0.5 rounded font-mono">Enter</kbd> {t('search.select')}</span>
          <span><kbd className="bg-secondary px-1 py-0.5 rounded font-mono">Esc</kbd> {t('search.close')}</span>
        </div>
      </div>
    </div>
  )
